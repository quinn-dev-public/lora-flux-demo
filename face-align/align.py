...
def prompt_user_to_choose_face(im, rects, filename=None):
    if filename and filename in FACE_CHOICES:
        return rects[FACE_CHOICES[filename]]
...
def get_landmarks(im, filename=None):
    ...
    if len(rects) == 0 and len(DETECTOR(im, 0)) > 0:
        rects = DETECTOR(im, 0)
    if len(rects) == 0:
        print("No faces found in the current image.")
        return None
    target_rect = rects[0] 
    if len(rects) > 1:
        target_rect = prompt_user_to_choose_face(im, rects, filename)
